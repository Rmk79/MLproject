...
modFit
```
From this we can expect an accuracy of around 80% though cross validation tends to over estimate this.
...
...and generate a confusion matrix on this test data and we can see that it is about 82% accurate here.

```{r}
confusionMatrix(test$classe,predict(modFit,testPCA))
```

I then loaded the test data provided on the class site and rand the predict method on it, but I won't show that here because it will give my answers away!
